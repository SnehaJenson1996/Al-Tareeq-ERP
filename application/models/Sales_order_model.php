 ...
 public function get_sales_order_master($so_id)
{
    return $this->db
        ->select('
            ...
            q.quotation_code,
            q.quotation_date,
            q.quotation_branch_id,
            q.project_name,
            q.project_location,

            COALESCE(c1.customer_name, c2.customer_name) as customer_name,
            COALESCE(c1.customer_TR_no, c2.customer_TR_no) as customer_TR_no,
            COALESCE(c1.contact_number, c2.contact_number) as contact_number,
            COALESCE(c1.emirate, c2.emirate) as emirate,
            COALESCE(c1.customer_address, c2.customer_address) as customer_address,
            COALESCE(c1.customer_email, c2.customer_email) as customer_email,
        ...
        ->join('customer_master c1', 'c1.customer_id = enq.enquiry_customer', 'left')

        // direct quotation
        ->join('customer_master c2', 'c2.customer_id = q.quotation_customer', 'left')

        // branch from quotation
        ->join('branch_master b', 'b.branch_id = q.quotation_branch_id', 'left')
    ...
    public function get_available_quantities($enquiry_id, $qtn_id, $current_so_id = null)
    {
        ...
        ')
            ...
            ->join('unit_master u', 'u.unit_id = qp.unit_id', 'left')
            ->where('qp.qtn_id', $qtn_id)
            ->get()
            ->result_array();

        if (empty($qtn_products)) {
            return [];
        }

        /* ================= 2) GET SALES ORDERS (BY QUOTATION) ================= */
        $this->db->select('so_id, so_code')
        ...
        $so_rows = $this->db->get()->result_array();
        ...
        /* ================= 4) CURRENT SO ADJUSTMENT ================= */
        $current_so_map = [];
        if ($current_so_id) {
            $current_products = $this->db
        ...
        foreach ($so_sums as $r) {
            ...
            if ($current_so_id && isset($current_so_map[$r['product_id']])) {
                $qty -= $current_so_map[$r['product_id']];
            }

            $so_map[$r['product_id']] = max(0, $qty);
        }

        /* ================= 6) CALCULATE AVAILABLE ================= */
        foreach ($qtn_products as &$p) {
            $prod_id = $p['prd_id'];
            $so_qty  = $so_map[$prod_id] ?? 0;

            $p['so_qty']         = $so_qty;
            $p['available_qty'] = max(0, (float)$p['qtn_qty'] - $so_qty);
            $p['available_total'] = $p['available_qty'] * $p['unit_price'];
    ...
    public function get_available_quantities_old($enquiry_id, $qtn_id, $current_so_id = null)
    {
        ...
        foreach ($qtn_products as &$p) {
            ...
            $p['available_total'] = $p['available_qty'] * $p['unit_price'];
            $p['so_codes'] = implode(', ', $so_codes);
            $p['so_discount_amount'] = $p['discount_amount'];
            // $p['so_taxable_amount'] = $p['taxable_amount'];
        }

        return $qtn_products;
    }
 public function get_edit_so_quantities($so_id, $qtn_id)
{
    $this->db->select('
        qi.prd_id,
        qi.unit_price,
    ...
    foreach ($quotation_items as $item) {
        ...
        $used_qty_row = $this->db->get()->row();
        $used_qty = (float)($used_qty_row->used_qty ?? 0);

        // CURRENT SO
        $this->db->select('quantity, amount, discount_amount');
        $this->db->from('sales_order_products');
        $this->db->where('so_id', $so_id);
        $this->db->where('product_id', $item_id);

        $current = $this->db->get()->row_array();

        $so_qty = (float)($current['quantity'] ?? 0);
        $so_amount = (float)($current['amount'] ?? 0);
        $so_discount = (float)($current['discount_amount'] ?? 0);

        // CALCULATION
        $available_qty = max(0, $quotation_qty - $used_qty);
        $max_qty = min($quotation_qty, $available_qty + $so_qty);
    ...
    public function get_sales_order_master_details($so_id)
    {
        return $this->db
            ...
            COALESCE(c.customer_TR_no, c2.customer_TR_no) AS customer_TR_no,
            COALESCE(c.contact_number, c2.contact_number) AS contact_number,
            COALESCE(c.emirate, c2.emirate) AS emirate,
            COALESCE(c.customer_address, c2.customer_address) AS customer_address,
            COALESCE(c.customer_email, c2.customer_email) AS customer_email,
   ...
   public function get_delivery_challan_list_data($enquiry_id = null, $quotation_id = null)
{
    ...
    // ✔ FIX NULL check properly
    $this->db->where('invoice_status IS NULL', null, false);
    ...
    public function get_direct_invoice_products($invoice_id)
{
    ...
    public function get_filtered_invoices($from_date, $to_date, $customer_id = '', $sales_person = '')
{
    $this->db->select('
        im.invoice_id,
        im.invoice_code,
    ...
    // Date filter
    $this->db->where('im.invoice_date >=', $from_date);
    $this->db->where('im.invoice_date <=', $to_date);

    // Customer filter
    if (!empty($customer_id)) {
        $this->db->where('im.invoice_customer', $customer_id);
    }

    // Salesperson filter (invoice creator)
    if (!empty($sales_person)) {
        $this->db->where('im.created_by', $sales_person);
    }

    // Only active invoices, not returned
    $this->db->where('im.invoice_status', 'Active');
    $this->db->where('im.sales_return !=', 1);
...
public function get_quotation_print_data($qtn_id)
{
    return $this->db
        ...
        ->join('branch_master b', 'b.branch_id = q.quotation_branch_id', 'left')
        ->join('customer_master c', 'c.customer_id = q.quotation_customer', 'left')
...
{
    $this->db->select('item_id, item_name, unit_price');
