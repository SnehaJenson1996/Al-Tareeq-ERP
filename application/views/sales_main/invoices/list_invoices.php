                  ...
                  <ul class="nav nav-tabs bar_tabs" id="myTab" role="tablist">
                    ...
                    <div class="tab-content" id="myTabContent">
                      ...
                      <table id="datatable-direct" class="table table-striped table-bordered" style="width:100%">
                        ...
                        <tbody>
                          <?php $i=1; foreach($direct_invoices as $invoice): ?>
                            <tr>
                              <th scope="row"><?= $i++; ?></th>
                              <td>
                              ...
                              <td>
                                <?php if(has_view_access($user, $page_name)){ ?>
                                  <a target='_blank' href='<?= base_url("index.php/Document_controller/print_invoice/".$invoice['invoice_id']."/".$invoice['customer_id']) ?>' title='Print'>
                      ...
                      <div class="tab-pane fade" id="sales-return" role="tabpanel" aria-labelledby="sales-return-tab">
                       <table id="datatable-responsive" class="table table-striped table-bordered" style="width:100%">
                            <thead>
                             ...
                             <tbody>
                                  <?php $i=1; foreach($sales_return_invoices as $invoice): ?>
                                    <tr>
                                      <th scope="row"><?= $i++; ?></th>
                                      <td><?= $invoice['invoice_code']; ?><br><?= date('d-M-Y', strtotime($invoice['invoice_date'])); ?></td>
                                      <td><?= $invoice['delivery_code']; ?></td>
    ...
    if (selected === "sales_return") {
      ...
      if (confirm("Are you sure you want to cancel this invoice?")) {
        $.ajax({
          ...
          type: "POST",
          data: { invoice_id: invoice_id },  // pass invoice id dynamically
