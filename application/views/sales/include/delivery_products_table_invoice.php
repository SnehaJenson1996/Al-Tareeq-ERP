...
if(!empty($so_products)){
    foreach($so_products as $so_p){
        if($so_p['item_id'] == $q['prd_id'] || $so_p['product_id'] == $q['prd_id']){
            $delivered = $so_p['so_qty'] ?? 0;
            break;
        }
    }
}
            ?>
                <tr>
                   ...
                   <td>
    <?= $so_p['so_discount_amount'] ?? 0 ?>
    <input type="hidden" name="discount_amount[]" value="<?= $so_p['so_discount_amount'] ?? 0 ?>">
</td>
                  <?php
$discount = $so_p['so_discount_amount'] ?? 0;

$taxable = ($delivered * $q['unit_price']) - $discount;
?>

<td>
    <?= number_format($taxable, 2) ?>
    <input type="hidden" name="taxable_amount[]" value="<?= $taxable ?>">
</td>
                </tr>
            <?php endforeach; ?>
        <?php else : ?>
