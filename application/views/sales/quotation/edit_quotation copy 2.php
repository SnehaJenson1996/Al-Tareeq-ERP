...
<body>
        ...
        <form action="<?= base_url('index.php/Sales/update_quotation') ?>" method="post" id="editQuotationForm">
                                                        ...
                                                        <?php foreach ($sub['products'] as $prod): ?>
                                                            <tr>
                                                                <td style="min-width:200px">
                                                                    ...
                                                                    <textarea name="products[<?= $mi ?>][<?= $sj ?>][<?= $pk ?>][product_description]" class="form-control mt-1" rows="2"><?= htmlspecialchars($prod['product_description'] ?? '') ?></textarea>
                                                                </td>
            ...
            <div class="row mt-4">
                <div class="col-md-6">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-8">
                            <input type="text" id="qtn_vat_amount" name="qtn_vat_amount" class="form-control" value="<?= $qtn_master['vat_amount'] ?? 0 ?>" readonly>
                        </div>
                    </div>
                </div>
            ...
            <div class="row mt-3">
                <div class="col-md-6">
                    ...
                    <textarea name="payment_term" class="form-control" rows="3"><?= $qtn_master['payment_term'] ?? '' ?></textarea>
                </div>
                <div class="col-md-6">
            ...
            <div class="row mt-2">
                <div class="col-md-6">
                    ...
                    <textarea name="delivery_term" id="delivery_term" class="form-control" rows="4"><?= $qtn_master['delivery_term'] ?? '' ?></textarea>
                </div>
                <div class="col-md-6">
                    <label>Terms & Conditions</label>
                    <textarea name="terms_condition" id="terms_condition" class="form-control" rows="4"><?= $qtn_master['terms_condition'] ?? '' ?></textarea>
                </div>
            </div>
    ...
    <script type="text/template" id="tpl_product">
        ...
        <td style="min-width:200px">
            ...
            <textarea name="products[{{MAIN}}][{{SUB}}][{{ROW}}][product_description]" class="form-control mt-1" rows="2"></textarea>
        </td>
        <td>
        ...
        <td><input type="number" step="0.01" name="products[{{MAIN}}][{{SUB}}][{{ROW}}][discount_amount]" class="form-control qtn_discount_amount" value="0"></td>
        ...
        $(function() {
            ...
            function recalcAll() {
                ...
                $('.qtn_amount').each(function() {
                    // use taxable fields for subtotal addition (as you used previously)
                    // but your earlier code used taxable amounts; use qtn_taxable_amount
                });
                $('.qtn_taxable_amount').each(function() {
                ...
                const addDiscAmt = ((subtotal + otherCharges) * addDiscPerc) / 100;
                $('#qtn_add_discount_amount').val(addDiscAmt.toFixed(2));

                const totalBeforeVat = subtotal + otherCharges - addDiscAmt;
                ...
                const vatAmt = applyVat ? (totalBeforeVat * vatPerc) / 100 : 0;
                $('#qtn_vat_amount').val(vatAmt.toFixed(2));

                const grand = totalBeforeVat + vatAmt;
