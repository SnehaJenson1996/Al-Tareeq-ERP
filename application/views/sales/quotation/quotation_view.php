<div class="text-end mb-3">
    ...
    <button type="button"
        ...
        onclick="window.location.href='<?= base_url('index.php/Sales/export_quotation_excel/' . $quotation['qtn_id']) ?>'">
    ...
    <div class="x_content">
        ...
        <table class="table table-bordered" style="width:100%; border-collapse:collapse; font-size:13px; margin-bottom:20px;">
            <tr>
                ...
                <td style="width:30%;"><?= $quotation_type == "sales" ? $quotation['enquiry_code'] : "--" ?></td>
            </tr>
            <tr>
                <th>Estimation</th>
                <td><?= $quotation_type == "sales" ? $quotation['estimation_code'] : "--" ?></td>
                <th>Branch</th>
...
<th style="width:15%;">Taxable</th>
            ...
            <tbody>
                <?php foreach ($quotation['main_headings'] as $main): ?>
                    ...
                    <tr style="background:#f2f2f2; font-weight:bold;">
                        ...
                        <td colspan="8"><?= $main['description'] ?></td>
                    </tr>
                    ...
                    <?php foreach ($main['sub_headings'] as $sub): ?>
                        ...
                        <tr style="background:#e8f1ff; font-style:italic;">
                            <td></td>
                            <td colspan="8"><?= $sub['sub_heading'] ?></td>
                        </tr>
                        ...
                        <?php foreach ($sub['products'] as $prd): ?>
                            <tr>
                                ...
                                <td><?= $prd['unit_name'] ?></td>
                                <td><?= $prd['quantity'] ?></td>
                                <td><?= isset($prd['unit_price']) && $prd['unit_price'] !== ''
                                        ? number_format((float)$prd['unit_price'], 2)
                                        : '-' ?></td>

                                <td><?= isset($prd['amount']) && $prd['amount'] !== ''
                                        ? number_format((float)$prd['amount'], 2)
                                        : '-' ?></td>
                                 <td><?= isset($prd['discount_amount']) && $prd['discount_amount'] !== ''
                                        ? number_format((float)$prd['discount_amount'], 2)
                                        : '-' ?></td>

                                 <td><?= isset($prd['taxable_amount']) && $prd['taxable_amount'] !== ''
                                        ? number_format((float)$prd['taxable_amount'], 2)
                                        : '-' ?></td>
            ...
            <tfoot>
               ...
               <tr style="background:#f9f9f9; font-weight:bold;">
    <td colspan="8" style="text-align:right;">Other Charges</td>
    <td><?= number_format((float)($quotation['totals']['other_charge'] ?? 0), 2) ?></td>
</tr>
                <tr style="background:#f9f9f9; font-weight:bold;">
                ...
                <tr style="background:#f9f9f9; font-weight:bold;">
                    <td colspan="8" style="text-align:right;">Total Before VAT</td>
                    <td><?= number_format($quotation['totals']['total_before_vat'], 2) ?></td>
                </tr>
                <?php if ($quotation['totals']['vat_required']): ?>
                    <tr style="background:#f9f9f9; font-weight:bold;">
                        <td colspan="8" style="text-align:right;">VAT (<?= $quotation['totals']['vat_percentage'] ?>%)</td>
