        ...
        foreach ($records as $r) { ?>
            <tr id="<?php echo $i; ?>">
                <td>
                    ...
                    <input type="hidden" class="form-control" name="product_id[]" value="<?php echo $r->product_id; ?>" />
                </td>
                <td><input type="text" class="form-control" name="description[]" value="<?php echo isset($r->description) ? $r->description : ''; ?>" /></td>
                <td><input type="number" class="form-control qty" name="item_quantity[]" id="item_quantity<?php echo $i; ?>" value="<?php echo $r->quantity; ?>" /></td>
                <td>
                    <select class="form-control" name="unit_name[]" style="min-width:120px;">
                        <option><?php echo $r->unit_name; ?></option>
                    </select>
                </td>
                <td><input type="number" class="form-control unit_price" name="unit_price[]" step="any" id="unit_price<?php echo $i; ?>" value="<?php echo isset($r->retail_price) ? $r->retail_price : 0; ?>" /></td>
