...
class Company extends CI_Controller
{
    ...
    public function save_customer()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($customer_id) {
                ...
                // === Optional: update customer with ledger_id (if schema allows)
                // $this->db->update('customers', ['ledger_id' => $ledger_id], ['id' => $customer_id]);
                ...
                if (!empty($contacts)) {
                    ...
                    if ($status) {
                        $this->session->set_flashdata('success', 'Customer and ledger saved successfully.');
                    } else {
    ...
    {
        $user = $this->session->userdata('user_id');
        if (!has_access($user, 'Company/list_customers', 'D')) {
            $data['title'] = 'Access Denied';
            $data['main_content'] = 'errors/access_control.php';
        } else {
            $id = $this->input->post("id");
    ...
    public function get_supplier_by_branch()
    {
        $branch_id = $this->input->post('branch_id');
        $customers = $this->Company_model->get_supplier_by_branch($branch_id);
        echo json_encode($customers);
    }
    //Supplier
