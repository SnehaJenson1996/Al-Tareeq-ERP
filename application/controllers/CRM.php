...
class CRM extends CI_Controller
{
	...
	function add_enquiry($status = "", $enqid = "", $survey_id = "")
	{
		...
		if (!has_access($user, 'CRM/list_enquiries', 'A')) {
			...
			switch ($status) {
				case "": // CASE 1: status is empty
					...
					$data['main_content']   = 'crm/add_enquiry.php';
					$this->load->view('includes/template', $data);
					break;
				...
				case "1": // CASE 2: status is 1	
					...
					$enquiry_data           = $this->Crm_model->get_enquiry_by_id($enqid);
				...
				case "2": // CASE 2: status is 2 inserted site survey	
					...
					$enquiry_data           = $this->Crm_model->get_enquiry_by_id($enqid);
					$branch_id              = $enquiry_data['branch_id'];
					$survey_data            = $this->Crm_model->get_survey_by_id($survey_id);
					$data['title']          = $enquiry_data['enquiry_code'];
					$data['status_info']    = "New";
					$data['menu_status']    = 2;
					$data['enquiry_data']   = $enquiry_data;
					...
					$data['all_products']   = $this->Item_model->get_all_item_list();
					$data['active_units']   = $this->Item_model->get_all_units();
					$data['survey_files']   = $this->Crm_model->get_survey_files_by_id($survey_data['survey_id']);
					///Nedd to get employee by designation 
					$des_id                 = 2;
					$data['employee_list']  = $this->Company_model->get_all_employees_designation_id($des_id, $branch_id);
					$data['site_survey']    = 1;
	...
	public function save_enquiry()
	{
		...
		if (!$this->_validate_enquiry_form()) {

			// Store error messages in flashdata
			...
			// Prepare data for view reload
			$data['title']        = 'New Enquiry';
			$data['status']       = 1;
			$data['menu_status']  = 1;

			$prefix               = 'AL' . date("y") . '-ENQ';
			$num                  = $this->Setup_model->get_next_code(
				$prefix,
				'enquiry_code',
				'enquiry_master',
				11
			...
			$this->load->view('includes/template', $data);
			  ...
			  // Step 2: Prepare enquiry data
        $data = $this->_prepare_enquiry_data();

        // Step 3: Insert into database
        $inserted_id = $this->Crm_model->insert_enquiry($data);

        // Step 4: Redirect always to Enquiry edit tab (menu_status = 1)
        if ($inserted_id) {
            $this->session->set_flashdata('success', 'Enquiry saved successfully.');
            redirect('CRM/add_enquiry/1/' . $inserted_id); // menu_status = 1
        } else {
	...
	public function accept_site_survey()
{
	...
	{
		$user				 		 = $this->session->userdata('user_id');
		if (!has_view_access($user, 'CRM/list_enquiries')) {
			$data['title']			 = 'Access Denied';
			$data['main_content']	 = 'errors/access_control.php';
		} else {
			$data['title']			= 'Enquiry List';

			$data['all_enquiries']  = $this->Crm_model->get_all_enquiry_list();

			$data['main_content']   = 'crm/list_enquiries.php';
		}
		$this->load->view('includes/template', $data);
	}
