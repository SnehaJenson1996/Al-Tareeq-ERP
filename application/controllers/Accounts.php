...
$data['branch_list']    = $this->Company_model->get_all_branches();
    ...
    $data['indirect_income']   = $this->Accounts_model->get_indirect_income($from, $to, $branch_id);
    $data['indirect_expense']  = $this->Accounts_model->get_indirect_expense($from, $to, $branch_id);

    // Calculate Sub-Totals safely handling negative/positive types
  ...
  public function printpayment()
  {
    ...
    // echo "<pre>"; print_r($all_voucher_rows); exit;
    ...
    $data['headerPath'] = base_url(ltrim($data['branch_header'], '/'));
    $data['footerPath'] = base_url(ltrim($data['branch_footer'], '/'));
    $prepared_by_id = $header->prepared_by ?? null;
    $approved_by_id = $header->approved_by ?? null;
...
public function trial_balance($from_date = null, $to_date = null)
{
    ...
    // Pass branch_id to model
    $data['accounts'] = $this->Accounts_model->get_account_trial_balance($from_sql, $to_sql, $branch_id);
    $data['group_totals'] = $this->Accounts_model->get_group_totals($from_sql, $to_sql, $branch_id);
  ...
  public function trial_balance_export()
  {
    ...
    $from_date = $this->input->post('from_date');
    $to_date = $this->input->post('to_date');
    $branch_id = $this->input->get('branch_id') ?? $this->input->post('branch_id');

    $this->load->model('Setup_model');
    $this->load->model('Accounts_model');
    // $comapny_records = $this->Setup_model->get_company_master_list();
    $trial_balance_data = $this->Accounts_model->get_account_trial_balance($from_date, $to_date, $branch_id);
...
public function view_cash_flow()
{
    ...
    $data['from_date'] = $from_date_input ?? date('d-m-Y', strtotime('first day of this month'));
    $data['to_date']   = $to_date_input ?? date('d-m-Y', strtotime('last day of this month'));
...
function add_expense_entry()
  ...
  function add_expense_entry_details()
  { //in use
    $data['title'] = "Expense Entry";
    $this->load->model('Accounts_model');
    $id = $this->Accounts_model->add_expense_entry();
    if ($id != '')
      $this->session->set_flashdata('success', 'Record Successfully Saved');
    redirect('accounts/view_expense_entry_list');
  }

  function view_expense_entry_list()
  { //in use
    $data['title'] = "Expense Entry List";
    if ($this->input->post('from')) {
      $data['from'] = $this->input->post('from');
      $data['to'] = $this->input->post('to');
    } else {
    ...
    $this->load->model('Accounts_model');
    $data['records'] = $this->Accounts_model->get_expense_entry_records($data['from'], $data['to']);
    $data['main_content'] = 'Accounts/expense_entry_list.php';
    $this->load->view('includes/template', $data);
  }
