...
class Sales extends CI_Controller
{
	...
	public function save_quotation()
	{
		...
		//$validation = validate_quotation($quotationData);

		// if (!$validation['status']) {
		// 	$this->session->set_flashdata('error', implode('<br>', $validation['errors']));
		// 	redirect('Sales/create_quotation');
		// 	return;
		// }
	...
	public function get_delivery_challan_invoice()
	{
		...
		$so_id = $del_master['so_id'];
		$so_master = $this->Sales_order_model->get_sales_order_master($so_id);
		$data['so_master'] = $so_master;
		$data['so_address'] = $this->Sales_order_model->get_so_address($so_id);

		// Enquiry Data
		$enquiry_data = $this->Sales_model->get_enquiry_by_id($del_master['enquiry_id']);
		...
		// Quotation
		$qtn_id = $del_master['quotation_id'];
		...
		// Products from Sales Order
		$so_result = $this->Sales_order_model->get_edit_so_quantities($so_id, $del_master['enquiry_id'], $qtn_id);
		$data['so_products'] = $so_result['products'];  // Products array
		$data['can_create_dc'] = $so_result['can_create_dc']; // Flag
	...
	function get_cust_accountId_from_quote($qtn_id)
	{
		...
		$this->load->model('Sales_model');
		$data['records1'] = $this->Sales_model->get_all_quotation_details($qtn_id);
...
// if ($delivery_master) {//use in edit
